drop database if exists shield_batch;
create database shield_batch Default Charset utf8mb4 collate utf8mb4_general_ci;
use shield_batch;
...
(
    ...
    allow_ip               varchar(255) comment '执行器ip: 任务执行的ip地址',
    allow_time             varchar(255) comment '允许最早执行时间: 格式: HH:mm:ss',
    maximum_limit          int comment '最大限制: 最大限制，当达到最大限制时，任务将不会再执行',
    api_url                varchar(255) comment '任务url: 调用的api地址，支持http和https',
    api_params             varchar(255) comment '任务参数: 调用api时传递的参数，支持json格式',
    format_func            varchar(255) comment '格式化函数: 格式化函数，用于格式化返回的数据: 00-表达式;01-Jar包',
    ...
    result_file            varchar(255) comment '结果文件: 结果文件，用于保存结果',
    result_flag            varchar(255) comment '结果标记: 结果标记，用于匹配结果',
    status                 int         default 0 comment '状态: 0-未启用;1-启用;',
    export_server_ip       varchar(255) comment '导出服务器ip: 导出文件时，需要指定导出服务器ip',
    export_server_port     int comment '导出服务器端口: 导出文件时，需要指定导出服务器端口',
    export_server_user     varchar(255) comment '导出服务器用户名: 导出文件时，需要指定导出服务器用户名',
    export_server_password varchar(255) comment '导出服务器密码: 导出文件时，需要指定导出服务器密码',
    export_server_path     varchar(255) comment '导出服务器路径: 导出文件时，需要指定导出服务器路径',
...
(
    ...
    start_time    timestamp    not null comment '实际开始时间',
    ...
    export_count  int          not null comment '导出数',
    export_file   varchar(255) not null comment '导出文件: 绝对路径',
